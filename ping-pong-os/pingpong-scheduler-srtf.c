...
int user_tasks_execution_time[USER_TASKS_MAX] = {30, 50, 70, 90, 110, 130}; // cenario 3
...
void Body (void * arg)
{
  ...
  while (taskExec->tempo_decorrido < task_get_eet(NULL)) {
    end_time--;
    if ((last_printed_line+5) <= systime()) {
      printf ("[%d]\t%s: interacao %d\t\t%d\n", systime(), (char *) arg, end_time, taskExec->tempo_decorrido) ;
      last_printed_line = systime();
    }
